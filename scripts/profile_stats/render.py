...
def build_reference_html(card: DashboardCardData | None = None) -> str:
    ...
    html_text = replace_exact(
        ...
        (
            ...
            f'                <div class="mono-tiny" style="position: absolute; top: calc(50% - 74px); left: 50%; transform: translateX(-50%); letter-spacing: 0.2em; white-space: nowrap; font-size: 0.72rem;">{window_label(card)}</div>'
        ),
        "center label",
    )
    html_text = replace_exact(
